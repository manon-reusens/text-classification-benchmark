...
name = 'svm_ft_silicone_emotion' #change here
...
def train_fasttext(config = None,
    ...
    with wandb.init(config=config,group=name) as run:
        save=path+name+sweep_id+run.project +'-model-'+run.name+str(datetime.now().time()).replace(':','')+'sav'
        ...
        wandb.log({"accuracy": accuracy, 'SEED':SEED, "f1 macro":f1_macro, "AUC-PC":aucpc, 'AUC':auc , 'SEED': SEED})
